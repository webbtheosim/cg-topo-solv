...
def encoder_desc_dnn(input_shape=(12,)):
    """
...
def encoder_desc_gnn(input_shape=[(100, 100), (12,)]):
    """
...
def regressor_dnn():
    ...
    r_out = layers.Dense(4, activation="tanh", name="r3")(x)
    # r_out = layers.Lambda(lambda z: tf.clip_by_value(z, -1, 1.0), name="r4")(r_out)
...
def decoder_cnn():
    ...
    x = layers.Conv2DTranspose(32, (3, 3), strides=2, padding='same', activation='relu', name="d5")(x)
    x = layers.Conv2DTranspose(32, (3, 3), strides=2, padding='same', activation='relu', name="d6")(x)
...
def get_callbacks(weight_path, monitor='val_decoder_acc', patience=200, verbose=0):
    """
    ...
    checkpoint = callbacks.ModelCheckpoint(weight_path, monitor=monitor, mode=mode, 
                                           save_weights_only=True, save_best_only=True, verbose=verbose)
...
def train_vae(enc_type, dec_type, monitor, if_reg, if_cls, x_train, x_valid, y_train, y_valid,
              ...
              lr=0.001, bs=32, if_train=False, n_class=6, n_epoch=1000, date='20240816', verbose=0):
    """
    ...
    if if_train:
        c1, c2 = get_callbacks(os.path.join(WEIGHT_DIR, weight_name), monitor=monitor, verbose=verbose)
        ...
        hist = model.fit(in_train, train_label, validation_data=(in_valid, valid_label),
                         callbacks=[c1, c2], epochs=n_epoch, verbose=verbose, batch_size=bs)
